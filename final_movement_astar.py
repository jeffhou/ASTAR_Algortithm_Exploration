...
while True:
	if 'Display':
		...
		obstacle_course=pygame.image.load('preview.png')
		windowSurface.blit(obstacle_course,(0,0))
		if len(path)>0:
			for i in path:
				pygame.draw.rect(windowSurface,(155,155,155),pygame.Rect(i[0]*5+1, i[1]*5+1, 3, 3))
	...
	if 'Events':
		for event in pygame.event.get():
			...
			if event.type == MOUSEBUTTONDOWN and event.button == 3:
				...
				open_list={start_coor:[None,0,manhattan_distance(start_coor,end_coor)]}
				del a
			if path==[] and end_coor!=start_coor:
		...
		if len(path)>0:
			while 1:
				...
				if new_location!=None:
					break
			(start.left,start.top)=(new_location[0]*5,new_location[1]*5)
			start_coor=(new_location[0],new_location[1])
	mainClock.tick(10)
'''
if 'Logic':
